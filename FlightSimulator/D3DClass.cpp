...
D3DClass::D3DClass()
{
	...
	m_pAlphaDisableBlendingState = 0;
}
	...
	float screenNear)
{
	...
	result = D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, 0,
		&featureLevel, 1, D3D11_SDK_VERSION, &swapChainDesc, &m_pSwapChain, &m_pDevice, NULL,
		&m_pDeviceContext);
	if (FAILED(result))
	{
		return false;
	...
	result = m_pDevice->CreateDepthStencilView(m_pDepthStencilBuffer, &depthStencilViewDesc,
		&m_pDepthStencilView);
	if (FAILED(result))
	{
		return false;
	...
	result = m_pDevice->CreateBlendState(&blendStateDescription, &m_pAlphaEnableBlendingState);
	if (FAILED(result))
	{
		return false;
	}
	...
	D3DXMatrixPerspectiveFovLH(&m_matProj, fieldOfView, screenAspect, screenNear, screenDepth);

	// Initialize the world matrix to the identity matrix.
	D3DXMatrixIdentity(&m_matWorld);

	// Create an orthographic projection matrix for 2D rendering.
	D3DXMatrixOrthoLH(&m_matOrtho, (float)screenWidth, (float)screenHeight, screenNear, screenDepth);
	return true;
}
...
{
	worldMatrix = m_matWorld;
	return;
}

void D3DClass::GetOrthoMatrix(D3DXMATRIX& orthoMatrix)
{
	orthoMatrix = m_matOrtho;
	return;
}
